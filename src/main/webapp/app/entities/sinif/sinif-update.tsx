...
import { getEntities as getOgrencis } from 'app/entities/ogrenci/ogrenci.reducer';
...
export const SinifUpdate = () => {
  ...
  const ogrencis = useAppSelector(state => state.ogrenci.entities);
  ...
  const saveEntity = values => {
    const entity = {
      ...
      ogrenci: ogrencis.find(it => it.id.toString() === values.ogrenci.toString()),
    };
  ...
  const defaultValues = () =>
    isNew
      ...
      : {
          ...
          ogrenci: sinifEntity?.ogrenci?.id,
        };
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="ex02App.sinif.home.createOrEditLabel" data-cy="SinifCreateUpdateHeading">
            <Translate contentKey="ex02App.sinif.home.createOrEditLabel">Create or edit a Sinif</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                label={translate('ex02App.sinif.sinifAdi')}
                id="sinif-sinifAdi"
                name="sinifAdi"
                data-cy="sinifAdi"
                type="text"
              />
              <ValidatedField
                label={translate('ex02App.sinif.sinifKodu')}
                id="sinif-sinifKodu"
                name="sinifKodu"
                data-cy="sinifKodu"
                type="text"
              />
              <ValidatedField label={translate('ex02App.sinif.brans')} id="sinif-brans" name="brans" data-cy="brans" type="select">
                {bransValues.map(brans => (
                  <option value={brans} key={brans}>
                    {translate('ex02App.Brans.' + brans)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                ...
                name="ogretmen"
                data-cy="ogretmen"
                label={translate('ex02App.sinif.ogretmen')}
                type="select"
              >
                <option value="" key="0" />
                {ogretmen
              ...
              <ValidatedField id="sinif-ogrenci" name="ogrenci" data-cy="ogrenci" label={translate('ex02App.sinif.ogrenci')} type="select">
                <option value="" key="0" />
                {ogrencis
                  ? ogrencis.map(otherEntity => (
                      <option value={otherEntity.id} key={otherEntity.id}>
                        {otherEntity.id}
                      </option>
                    ))
