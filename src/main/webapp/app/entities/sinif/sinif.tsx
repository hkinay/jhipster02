  ...
  return (
    <div>
      <h2 id="sinif-heading" data-cy="SinifHeading">
        <Translate contentKey="ex02App.sinif.home.title">Sinifs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="ex02App.sinif.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/sinif/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="ex02App.sinif.home.createLabel">Create new Sinif</Translate>
          </Link>
        </div>
      </h2>
        ...
        {sinifList && sinifList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="ex02App.sinif.id">ID</Translate>
                </th>
                <th>
                  <Translate contentKey="ex02App.sinif.sinifAdi">Sinif Adi</Translate>
                </th>
                <th>
                  <Translate contentKey="ex02App.sinif.sinifKodu">Sinif Kodu</Translate>
                </th>
                <th>
                  <Translate contentKey="ex02App.sinif.brans">Brans</Translate>
                </th>
                <th>
                  <Translate contentKey="ex02App.sinif.ogretmen">Ogretmen</Translate>
                </th>
                <th>
                  <Translate contentKey="ex02App.sinif.ogrenci">Ogrenci</Translate>
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {sinifList.map((sinif, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`ex02App.Brans.${sinif.brans}`} />
                  </td>
                  <td>{sinif.ogretmen ? <Link to={`/ogretmen/${sinif.ogretmen.id}`}>{sinif.ogretmen.id}</Link> : ''}</td>
                  <td>{sinif.ogrenci ? <Link to={`/ogrenci/${sinif.ogrenci.id}`}>{sinif.ogrenci.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ex02App.sinif.home.notFound">No Sinifs found</Translate>
            </div>
          )
        )}
      </div>
